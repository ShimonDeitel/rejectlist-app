import XCTest
@testable import Rejectlist

@MainActor
final class RejectlistStoreTests: XCTestCase {

    func makeFreshStore() -> RejectlistStore {
        // Use a fresh store; it will load persisted state or seed data.
        let store = RejectlistStore()
        return store
    }

    func testSeedDataLoadsBelowFreeLimit() {
        let store = makeFreshStore()
        XCTAssertLessThan(store.items.count, RejectlistStore.freeLimit)
    }

    func testAddIncreasesCount() {
        let store = makeFreshStore()
        let before = store.items.count
        let added = store.add(title: "Test Entry", category: "Reach", value: 1.0)
        XCTAssertTrue(added)
    ...
    func testFreeLimitBlocksAdditionalAdds() {
        let store = makeFreshStore()
        while store.items.count < RejectlistStore.freeLimit {
            _ = store.add(title: "Filler", category: "Reach", value: 1.0)
        }
        let added = store.add(title: "Overflow", category: "Reach", value: 1.0)
        XCTAssertFalse(added)
        XCTAssertEqual(store.items.count, RejectlistStore.freeLimit)
    }
    ...
    func testProBypassesFreeLimit() {
        ...
        let added = store.add(title: "Extra", category: "Reach", value: 1.0)
        XCTAssertTrue(added)
    }

    func testDeleteRemovesItem() {
        let store = makeFreshStore()
        _ = store.add(title: "ToDelete", category: "Reach", value: 1.0)
    ...
    func testTotalValueSumsItems() {
        ...
        _ = store.add(title: "SumTest", category: "Reach", value: 5.0)
        XCTAssertEqual(store.totalValue, before + 5.0, accuracy: 0.001)
    }

    func testToggleResolvedFlipsFlag() {
        let store = makeFreshStore()
        _ = store.add(title: "ResolveMe", category: "Reach", value: 1.0)
